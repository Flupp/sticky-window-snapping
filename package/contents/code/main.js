...
function clientStartUserMovedResized(client) {
	...
	var l1 = compat.Window_frameGeometry(client).x;
	var r1 = compat.Window_frameGeometry(client).width + l1;
	var t1 = compat.Window_frameGeometry(client).y;
	var b1 = compat.Window_frameGeometry(client).height + t1;
	...
	resizedClientInfo = { lOrig : l1   , rOrig : r1   , tOrig : t1   , bOrig : b1
	                    ...
	                    , wShake: {count: 0, direction: 0, val: compat.Window_frameGeometry(client).width , turnVal: compat.Window_frameGeometry(client).width }
	                    , hShake: {count: 0, direction: 0, val: compat.Window_frameGeometry(client).height, turnVal: compat.Window_frameGeometry(client).height} };
	var clients = compat.workspace_windowList();
	for (var i = 0; i < clients.length; i++) {
		var c = clients[i];
		var g = compat.Window_frameGeometry(c);
		var l2 = g.x;
		var r2 = g.width + l2;
		var t2 = g.y;
		var b2 = g.height + t2;
		...
		var snap = {
			...
			originalGeometry: shallowCopy(compat.Window_frameGeometry(c))
		};
...
function setGeometry(client, geometry, pinRightInsteadLeft, pinBottomInsteadTop) {
	var minSize = {
		w: Math.max(sanitizeQSize(client.minSize).w, Math.min(50, compat.Window_frameGeometry(client).width)),
		h: Math.max(sanitizeQSize(client.minSize).h, Math.min(50, compat.Window_frameGeometry(client).height))
	};
	...
	var left   = Math.min(ca.x, compat.Window_frameGeometry(client).x);
	var top    = Math.min(ca.y, compat.Window_frameGeometry(client).y);
	var right  = Math.max(ca.x + ca.width , compat.Window_frameGeometry(client).x + compat.Window_frameGeometry(client).width );
	var bottom = Math.max(ca.y + ca.height, compat.Window_frameGeometry(client).y + compat.Window_frameGeometry(client).height);
	...
	if (shallowEquals(compat.Window_frameGeometry(client), geometry)) {
		return false;
	} else {
